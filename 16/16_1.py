    ...
    while len(frontier) > 0:
        ...
        for n in next_to_explore_dirs:
            ...
            if cur_dir == n:
                ...
                if (cur_dir[0] != 0 and cur_dir[0] == -n[0]) or (cur_dir[1] != 0 and cur_dir[1] == -n[1]):
                    new_score = cur_score + 2001
                else:
                    new_score = cur_score + 1001
            if new_f == e_pos:
                path_scores.append((new_f, new_score,n))
            else:
                if memo[new_f[0]][new_f[1]] < new_score:
                    # another path has a better way
                    continue
                else:
                    memo[new_f[0]][new_f[1]] = new_score
                    frontier.append((new_f, new_score, n, new_path))

    frontier_to_return = []
    if len(path_scores) == 0:
        return frontier_to_return
    min_score = min([s for (_, s, _) in path_scores])
    print(f"{min_score=}")
    for d, score, p in path_scores:
        if score == min_score:
            frontier_to_return.append((score, d, p))
    return frontier_to_return
